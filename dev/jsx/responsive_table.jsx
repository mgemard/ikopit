...
var ResponsiveTable = React.createClass({
	getInitialState: function() {
     ...
     render: function() {
		...
		for (var i = 0; i < this.state.nbColumn; i++) {
			...
			//console.log(column);
				if (column == 'titanium' || column == 'copper' || column == 'iron' || column == 'aluminium' || 
					...
					rowHeader.push(<td key={column}><div className={"icon-"+column}></div></td>);
				} 
				else {
					rowHeader.push(<td className="title-res" key={column}>{this.props.columns[column]}</td>);
				}
    	}
		...
		//console.log(line);
			...
			for (var i = 0; i < this.state.nbColumn; i++) {
				...
				if (column == 'image') {
					...
					row.push(<td key={key+"-"+column}><div className={"icon-"+line.name}></div></td>);
				}
				else if (column == 'name') {
					row.push(<td key={key+"-"+column}>{this.props.name[line.name]}</td>);
				}
				else if (column == 'time') {
					row.push(<td key={key+"-"+column}>{this.getTime(line[column])}</td>);
				}
				else if (column == 'titanium' || column == 'copper' || column == 'iron' || column == 'aluminium' || 
								 column == 'silicon' || column == 'uranium' || column == 'nitrogen' || column == 'hydrogen') {
					row.push(<td key={key+"-"+column}>{numberWithCommas(this.getResCost(line[column]))}</td>);
				}
				else if (column == 'cost') {
					row.push(<td key={key+"-"+column}>{this.getCost(line[column])}</td>);
				}
				else if (column == 'upkeep') {
					row.push(<td key={key+"-"+column}>{this.getUpkeep(line[column])}</td>);
				}
				else if (column == 'agility') {
					row.push(<td key={key+"-"+column}>{line[column]}</td>);
				}
				else if (column == 'totCost') {
					row.push(<td key={key+"-"+column}>{this.getTotCost(line.titanium,line.copper,line.iron,
						line.aluminium,line.silicon,line.uranium,line.nitrogen,line.hydrogen)}</td>);
				}
				else if (column == 'impulse') {
					if (this.props.comp) {
						row.push(<td key={key+"-"+column}>{numberWithCommas(line[column])}</td>);
					}
					else if (line.name == 'adam') {
						row.push(<td key={key+"-"+column}>{"x"+line[column]} (+4.000)</td>);
					}
					else {
						row.push(<td key={key+"-"+column}>{"x"+line[column]}</td>);
					}					
				}
				else if (column == 'warp') {
					if (this.props.comp) {
						...
						row.push(<td key={key+"-"+column}>{"x"+line[column]} (+2.000)</td>);
					}
					else {
						row.push(<td key={key+"-"+column}>{"x"+line[column]}</td>);
					}					
    			...
    			if (line[column].length == 2) {
    				row.push(<td key={key+"-"+column}>{numberWithCommas(line[column][0])}/{numberWithCommas(line[column][1])}</td>);
    			}
    			else {
    			  row.push(<td key={key+"-"+column}>{numberWithCommas(line[column])}</td>);
    			}
    	...
    	rowBody.push(<tr id={line.name} onClick={this.handleClick.bind(this,key)} key={key}>{row}</tr>);
    	row = [];
			...
			if(this.state.nbColumn < this.state.nbColumnMax) {
			    for (var i = this.state.nbColumn; i < this.state.nbColumnMax; i++) {
	      			...
	      			wrapedResColumn.push( <div key={column} className={"res-mini "+column}>{numberWithCommas(this.getResCost(line[column]))}</div> );
	      		}
					...
					if (this.state.showDetails[key]) {
						if (!!wrapedColumn) {
							...
							rowBody.push(
				    		<tr key={key+"-wrap"}>
				    			<td colSpan={this.state.nbColumn}> {wrapedResColumn}</td>
				    		</tr>);
				    }
			    }
			}
